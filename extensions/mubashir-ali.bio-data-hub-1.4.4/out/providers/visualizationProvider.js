...
class VisualizationProvider {
    constructor(extensionUri) {
        this._csvDataProvider = new csvDataProvider_1.CSVDataProvider();
        this._extensionUri = extensionUri;
    }
    async showVisualizationOptions(uri) {
        try {
            ...
            let xAxis, yAxis, colorBy;
            switch (visualizationType) {
                case "Scatter Plot": {
                    xAxis = await vscode.window.showQuickPick(numericColumns, { placeHolder: "Select X-axis column" });
                    if (!xAxis)
                        return;
                    yAxis = await vscode.window.showQuickPick(numericColumns.filter((col) => col !== xAxis), { placeHolder: "Select Y-axis column" });
                    if (!yAxis)
                        return;
                    colorBy = await vscode.window.showQuickPick(["None", ...columns], {
                        placeHolder: "Select column for color coding (optional)",
                    });
                    this.showScatterPlot(uri, data, xAxis, yAxis, colorBy === "None" ? undefined : colorBy);
                    break;
                }
                case "Bar Chart": {
                    ...
                    if (!categoryColumn)
                        return;
                    const valueColumnBar = await vscode.window.showQuickPick(numericColumns.filter((col) => col !== categoryColumn), { placeHolder: "Select value column" });
                    if (!valueColumnBar)
                        return;
                    this.showBarChart(uri, data, categoryColumn, valueColumnBar);
                    break;
                }
                case "Line Chart": {
                    ...
                    if (!xAxisLine)
                        return;
                    const yAxisLine = await vscode.window.showQuickPick(numericColumns.filter((col) => col !== xAxisLine), { placeHolder: "Select Y-axis column" });
                    if (!yAxisLine)
                        return;
                    this.showLineChart(uri, data, xAxisLine, yAxisLine);
                    break;
                }
            }
        }
        catch (err) {
            vscode.window.showErrorMessage(`Error preparing visualization: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    ...
    showScatterPlot(uri, data, xAxis, yAxis, colorBy) {
        const panel = vscode.window.createWebviewPanel("csvVisualization", `Scatter Plot: ${uri.path.split("/").pop()}`, vscode.ViewColumn.Beside, {
            enableScripts: true,
        });
        panel.webview.html = this.getScatterPlotHtml(data, xAxis, yAxis, colorBy);
    }
    showBarChart(uri, data, categoryColumn, valueColumn) {
        const panel = vscode.window.createWebviewPanel("csvVisualization", `Bar Chart: ${uri.path.split("/").pop()}`, vscode.ViewColumn.Beside, {
            enableScripts: true,
        });
        panel.webview.html = this.getBarChartHtml(data, categoryColumn, valueColumn);
    }
    showLineChart(uri, data, xAxis, yAxis) {
        const panel = vscode.window.createWebviewPanel("csvVisualization", `Line Chart: ${uri.path.split("/").pop()}`, vscode.ViewColumn.Beside, {
            enableScripts: true,
        });
        panel.webview.html = this.getLineChartHtml(data, xAxis, yAxis);
    }
    getScatterPlotHtml(data, xAxis, yAxis, colorBy) {
        ...
        let colorScale = "";
        if (colorBy) {
            colorValues = [...new Set(plotData.map((point) => point.color).filter(Boolean))];
    ...
    getBarChartHtml(data, categoryColumn, valueColumn) {
    ...
    getLineChartHtml(data, xAxis, yAxis) {
