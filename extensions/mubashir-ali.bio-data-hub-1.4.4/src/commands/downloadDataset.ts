...
// Removed unused 'https' and 'stream' imports
 ...
 */
export async function downloadDataset(dataset: OnlineDataset) {
      ...
      try {
        ...
        vscode.window
          .showInformationMessage(
            `Dataset "${dataset.title}" downloaded successfully to ${datasetDir}`,
            "Open Folder"
          )
          .then((selection: string | undefined) => { // Added type annotation
 ...
 */
async function downloadZenodoDataset(
  dataset: OnlineDataset,
  targetDir: string,
  progress: vscode.Progress<{ increment?: number; message?: string }>,
  token: vscode.CancellationToken
