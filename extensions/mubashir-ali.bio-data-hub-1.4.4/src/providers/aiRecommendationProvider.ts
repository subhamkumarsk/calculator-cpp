...
// Define interfaces for dataset metadata
...
export class AIRecommendationProvider {
  ...
  private datasetIndex: DatasetMetadata[] = []
  ...
  async showRecommendationInterface() {
    ...
    if (this.datasetIndex.length === 0) {
      const rebuildIndex = await vscode.window.showInformationMessage(
        "No datasets have been indexed yet. Would you like to index your workspace now?",
        "Yes",
        "No",
      )
  ...
  private async buildDatasetIndex() {
  ...
  private async extractMetadata(filePath: string): Promise<DatasetMetadata> {
  ...
  private async getRecommendations(query: string) {
  ...
  private calculateRelevanceScore(dataset: DatasetMetadata, queryKeywords: string[]): number {
